...
import { useEleveCours } from "@/services/hooks/repetiteur";
...
interface RepetiteurCourseListProps {
  eleve: Eleve;
  isEleveReady: boolean;
}

export function RepetiteurCourseList({ eleve, isEleveReady }: RepetiteurCourseListProps) {
  const router = useRouter();
  const { data, isLoading, isError } = useEleveCours(isEleveReady);
        ...
        <div className="flex flex-col items-center gap-6 sm:gap-8 mt-4 sm:mt-8">
          ...
          <div className="text-center px-4">
            <p className="text-gray-600 text-base sm:text-lg mb-4 sm:mb-6">
              Cliquez ci-dessous pour générer un cours adapté au niveau de {eleve.prenom}
            </p>
        ...
        <div className="bg-[#F0F7EC] border border-[#C8E0B8] rounded-xl p-4 mb-4">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-gray-600">Cours de l'élève</p>
                <h3 className="text-lg font-semibold text-[#548C2F]">
                  {eleve.prenom} {eleve.nom}
                </h3>
              </div>
